...
public class StringTest2 {
	...
	public static void main(String[] args) {
		...
		System.out.println(t.trim().length()); //공백제거 후 출력

		
		// String클래스의 메서드 사용법
		String xyz="helloworld";
		char[]xyz2 = xyz.toCharArray(); //문자열 하나하나 뽑아서 캐릭터형테의 배열 생성
		//h가 1번방 e가 2번방 ... d가 마지막방
		for(int i=0;i<xyz2.length;i++) {
			System.out.println(xyz2[i]);
		}
		
		
		//기존문자열을 지우거나 붙이거나 Stringbuffer, Stringbuilder
		...
		boolean kkk = s.contains("h"); //h글자가 포함되어있는지 아닌지
		System.out.println(kkk);
		boolean kkk2 = s.contains("hx");
		System.out.println(kkk2);
		
		boolean kkk3 = s.endsWith("o"); //맨 끝에 글자가 o인지 확인
		...
		String ppp = String.format("%s %d", "홍길동",20); //두 taxt를 합침
		
		int index = "hello".indexOf("a");
		System.out.println(index);
		
		System.out.println("".isEmpty()); //""으로 된 문자열이 비어있는지 확인
		System.out.println("".length()); //현재 얘의 문자열은 0
