...
//		System.out.println(studentArray[i].studentInfo());  //stuInfo 생성해야함
		...
		if( studentArray[maxAgeIndex].getAge() 
				< studentArray[i].getAge()) {maxAgeIndex = i;}
		if( studentArray[minAgeIndex].getAge() 
				> studentArray[i].getAge()) {minAgeIndex = i;}
		if( studentArray[maxHeightIndex].getHeight() 
				< studentArray[i].getHeight()) {maxHeightIndex = i;}
		if( studentArray[minHeightIndex].getHeight() 
				> studentArray[i].getHeight()) {minHeightIndex = i;}
		if( studentArray[minWeightIndex].getWeight()
				< studentArray[i].getWeight()) {minWeightIndex = i;}
		if( studentArray[minWeightIndex].getWeight()
				> studentArray[i].getWeight()) {minWeightIndex = i;}
		}
	System.out.println();
	...
	System.out.println("몸무게가 가장 적게 나가는 학생 : " +	studentArray[minWeightIndex].getName());
		...
		StudentService service = new StudentService() ;
			Student stu = null;
			try {
				stu = service.getAStudentInfor(studentArray, "홍길동", 19);
				System.out.println("검색결과 "+stu+"=========");
			} catch (StudentNotFoundException e) {
				System.out.println(e.getMessage());
			}
			
			System.out.println(service.getAgeAvg(studentArray));
			System.out.println(service.getHegithAvg(studentArray));
			System.out.println(service.getWeithAvg(studentArray));
	}
}
